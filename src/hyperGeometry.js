...
export const project = (tile) => {
    ...
    for(let i=0; i < attr.length; i+=3) {
        ...
        // Beltrami-Klein
            attr[i  ] = x/y;
            attr[i+2] = z/y;
            attr[i+1] = -tile.geometry.userData.euclidean[i/3].y / y
        // Poincare
            // attr[i  ] = x/(y+1);
            // attr[i+2] = z/(y+1);
            // attr[i+1] = -tile.geometry.userData.euclidean[i/3].y/(y+1)
    }
}